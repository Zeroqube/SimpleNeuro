...
class NeuroNet:
    ...
    def learn(self, inputs, outputs, speed_of_learning):
        ...
        dirivative_of_activation_func = np.vectorize(lambda x: 1 - x ** 2)
        for i in range(1, len(self.layers_size)):
            in_layer_err = curr_err * dirivative_of_activation_func(results[-i])
    ...
    def learn_on_range_of_data(self, inputss, outputss, speed_of_learning):
        size_of_data_range = len(inputss)
